...
================================================================================
Version     : 19.0.0
Changes     : 1. COMMITTEEFUNCTION columns (COMMITTEEFUNCTIONA, COMMITTEEFUNCTIONC,
                 COMMITTEEFUNCTIONE, COMMITTEEFUNCTIONG, COMMITTEEFUNCTIONN,
                 COMMITTEEFUNCTIONRISK, COMMITFUNCTIONHEALTHSAFETY, COMMITTEEFUNCTIONO):
                 Any value of "Yes" (from CSV or DVV) is written as "1".
                 Values already "1" are kept as "1".
              2. All data cells are top-aligned and left-aligned.
              3. Individual Name column inserted immediately after serial_id
                 in every template sheet that has a serial_id column.
                 The name is read from REL_INDIVID in the extraction CSV,
                 mapped by the Series ID column nearest to REL_INDIVID.
              4. All v18 fixes retained (best-match section detection, etc).
...
def populate_template(
    ...
    for sheet_name in wb.sheetnames:
        ...
        # ── POSITION / SERIES: one row per extraction row ──────────────────
            ...
            # Find the Series ID column whose immediately following section
            # contains the MOST template header matches.
            # Using the first match is wrong because some data lib codes
            # (e.g. A_COMMITTEEMEMBERRETIRED) appear in multiple CSV sections.
            # The correct section is the one with the most template header hits.
